...
echo "=== Compilazione per Linux (arm) ==="
rustup target add aarch64-unknown-linux-gnu
cargo build --release --target aarch64-unknown-linux-gnu

# Verifica se il binario Linux esiste
LINUX_BINARY_PATH="target/aarch64-unknown-linux-gnu/release/$PACKAGE_NAME"
if [ -f "$LINUX_BINARY_PATH" ]; then
    echo "Binario Linux compilato con successo: $LINUX_BINARY_PATH"
    cp "$LINUX_BINARY_PATH" "build/${PACKAGE_NAME}-linux-arm64"
else
