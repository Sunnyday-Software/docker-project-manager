...
pub fn register_env_commands(registry: &mut CommandRegistry) {
    ...
    registry.register_closure_with_help_and_tag(
        ...
        |args, ctx| {
            ...
            match env::current_dir() {
                Ok(path) => {
                    debug_log(ctx, "rust-env", &format!("current directory retrieved: {}", path.display()));
    ...
    registry.register_closure_with_help_and_tag(
        ...
        |args, ctx| {
            ...
            match env::current_exe() {
                Ok(path) => {
                    debug_log(ctx, "rust-env", &format!("executable path retrieved: {}", path.display()));
                    Ok(Value::Str(path.to_string_lossy().to_string()))
                },
    ...
    registry.register_closure_with_help_and_tag(
        ...
        |args, ctx| {
            ...
            match dirs::home_dir() {
                Some(path) => {
                    debug_log(ctx, "rust-env", &format!("home directory retrieved: {}", path.display()));
                    Ok(Value::Str(path.to_string_lossy().to_string()))
                },
    ...
    registry.register_closure_with_help_and_tag(
        ...
        |args, ctx| {
            ...
            debug_log(ctx, "rust-env", &format!("collected {} environment variables", count));
            Ok(Value::List(vars))
        },
    );
}
